                ...
                <div class="game--answers" v-if="round.multiple">
                    <div class="game--answers--answer" v-for="answer in round.answers">
                        ...
                        <div class="game--answers--remove">
                            <button @click="removeAnswer(round, answer)">Remove</button>
                        </div>
                    </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            addTeam() {
                this.$set(this.game, 'teams', this.game.teams.concat([{ name: '', initialScore: 0 }]));
            },
            removeTeam(team) {
                this.$delete(this.game.teams, this.game.teams.indexOf(team));
            },
            addRound() {
                this.$set(this.game, 'rounds', this.game.rounds.concat([{
                    question: '',
                    answer: '',
                    answers: [],
                    multiple: false
            ...
            addAnswer(round) {
                this.$set(round, 'answers', round.answers.concat([{ text: '', isCorrect: false, }]));
            },
